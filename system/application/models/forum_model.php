...
WHERE `t`.`board_id` = ".$id." GROUP BY `t`.`id` ORDER BY `t`.`sticked` DESC, `post_time` DESC LIMIT ".$page.", ".$config['threadsLimit'].";")->result_array();
	...
	public function stickThread($id) {
		$this->load->database();
		$this->db->query("UPDATE `threads` SET `sticked` = '1' WHERE `id` = '".$id."'");
	}
	
	public function unstickThread($id) {
		$this->load->database();
		$this->db->query("UPDATE `threads` SET `sticked` = '0' WHERE `id` = '".$id."'");
	}
	
	public function closeThread($id) {
		$this->load->database();
		$this->db->query("UPDATE `threads` SET `closed` = '1' WHERE `id` = '".$id."'");
	}
	
	public function openThread($id) {
		$this->load->database();
		$this->db->query("UPDATE `threads` SET `closed` = '0' WHERE `id` = '".$id."'");
	}
	
	public function getFirstPost($id) {
		$this->load->database();
		return $this->db->query("SELECT `id` FROM `posts` WHERE `thread_id` = '".$id."' ORDER BY `id` ASC LIMIT 1")->result_array();
	}
	
	public function truncateThread($id, $first) {
		$this->load->database();
		$this->db->query("DELETE FROM posts WHERE thread_id = $id AND id != $first");
	}
}
